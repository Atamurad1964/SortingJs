...
function sortByRating(arr){
    // Делаем глубокое копирвание обьекта items
    const temp = JSON.parse(JSON.stringify(arr))

    //Здесь мы пробежимся по массиву temp и с помощью регулярных выражений '/\D/g' избавляемся  
    //от строчных значений в частности от слов "отзывов и у нас остаются только цифры"
    temp.forEach(element => {
        element.ratingRevievs = +element.ratingRevievs.replace(/\D/g, '')
    });
    //Теперь необходимо отсортировать
    temp.sort((a,b) => a.ratingRevievs > b.ratingRevievs ? 1 : -1)
    // Получаем наш див и обнуляем если там есть какие то данные
    document.querySelector('.result').innerHTML = ''

    // Перебираем каждый элемент массива и вытащим те элементы которые нам нужны
    // и формируем html структуру,верстку внутри нашего цикла
    temp.forEach(item =>{
 ...
 //Получаем кнопку при нажатии вызовем функцию sortByRating(items)
